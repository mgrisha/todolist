...
const StoreContextProvider = (props) => {
  ...
  const fetchData = async () => {
    let response = await axios("https://e485b0084c42382b.mokky.dev/todos");
  ...
  const deleteTodoItem = async (id) => {
    if (window.confirm("Ви впевнені, що хочете видалити це завдання?")) {
      await axios.delete(`https://e485b0084c42382b.mokky.dev/todos/${id}`);
  ...
  const handleChangeStatus = async (id) => {
    ...
    await axios.patch(`https://e485b0084c42382b.mokky.dev/todos/${id}`, {
      status: findTodoItem.status,
    });
